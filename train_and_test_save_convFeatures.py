...
def _train_or_test(args, model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True,
    ...
    for i, (image, label) in enumerate(dataloader):
        ...
        with grad_req:
            ...
            """ 
            # reshape from (batch_size, num_features, height, width) to (batch_size, num_features*height*width)
            ...
            image_names = [dataloader.dataset.samples[idx][0] for idx in range(i*dataloader.batch_size, (i+1)*dataloader.batch_size)]
            image_names = [os.path.basename(path) for path in image_names]  # Extract name from path

            # Convert to DataFrame
            df = pd.DataFrame(flat_features_np)

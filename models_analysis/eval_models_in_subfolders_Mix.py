...
def get_depth_directories(path, current_depth, target_depth):
    if current_depth == target_depth:
        return [path] if ("run" in path) else []
...
def get_pending_evaluations_list(full_list, subtraction_list):
    ...
    for key in full_list:
        ...
        if key in subtraction_list:
            difference_dictionary[key] = [value for value in full_list[key] \
                                          if value not in subtraction_list[key]]
        else:
            difference_dictionary[key] = [value for value in full_list[key] ]
        if "run10" in key:
            key_path = current_path +"/saved_models/"+ key[:5] +"/"+ key[6:] 
        else:
            key_path = current_path +"/saved_models/"+ key[:4] +"/"+ key[5:]
        pending_evaluations_list[key_path] = []
        for test in difference_dictionary[key]:
            test_path = current_path + "/datasets/" + test[:3] + "/" + test[4:] 
            #"D:/Results_trained_models/ProtoPNets/ISBI_2023/datasets/" + test[:3] + "/" + test[4:]
...
save_analysis_path = "./models_analysis"
...
imgdir_list = [
    base_dir + "datasets/Mix/"
    ]
""" base_dir + "datasets/Mix/test",
    base_dir + "datasets/Mix/test_brightness_0.8", 
    base_dir + "datasets/Mix/test_contrast_0.45",
    base_dir + "datasets/Mix/test_grey", 
    base_dir + "datasets/Mix/test_hue_-0.1",
    base_dir + "datasets/Mix/test_hue_-0.05",
    base_dir + "datasets/Mix/test_hue_-0.03",
    base_dir + "datasets/Mix/test_hue_-0.01",
    base_dir + "datasets/Mix/test_hue_0.01",
    base_dir + "datasets/Mix/test_hue_0.03",
    base_dir + "datasets/Mix/test_hue_0.05",
    base_dir +  "datasets/Mix/test_hue_0.1",
...
for model_path in subdirs:
    ...
    for imgdir in imgdir_list:
        ...
        complete_evaluations_list[model_name].append(imgdir)
...
pending_evaluations_list = complete_evaluations_list
...
for key_model_path, value_test_path in pending_evaluations_list.items():
    ...
    if highest_filename:
        ...
        for imgdir in value_test_path:
            ...
            csv_file_name= imgdir.split("/")[-2] + "_" + imgdir.split("/")[-1] + "_" + \
                  modeldir.split("/")[-1] + "_" + modeldir.split("/")[-2] #+ ".csv"

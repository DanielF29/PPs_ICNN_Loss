...
#def get_pps_icnn_loss(args, num_classes, supp_fts, query_fts, way, qry_labels):
def get_pps_icnn_loss(pps_fts, input_fts, num_classes, pps_per_class, input_labels):

    # We have the equivalency of:
    #    # The number of ways  = the number of classes
    # Then ways become num_classes
    #    # The number of shots = the umber of PPs per class
    # Then supp_fts becomes pps_fts
    #    # Not exactly but: 
    #    # The number of query images = the number of images in the input batch 
    # Then query_fts becomes input_fts

    loss = 0
    #features = torch.cat(pps_fts, input_fts)
    features = torch.cat((pps_fts, input_fts))
    #supp_labels = torch.arange(0, way, 1/args.shot).type(torch.int).cuda()
    pps_labels = torch.arange(0, num_classes, 1/pps_per_class).type(torch.int).cuda()
    #print("pps_labels shape: ", pps_labels.shape, "input_labels shape: ", input_labels.shape)
    labels = torch.cat((pps_labels, input_labels))
    #ipc = args.shot + args.query #son integers
    ipc = len(input_fts) 

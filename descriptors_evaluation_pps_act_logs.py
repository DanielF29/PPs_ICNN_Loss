...
from preprocessing_for_descriptors.mod_values import bright_lvl, contr_lvl, sat_lvl, hue_lvl, A, w, texture_h
#from log import create_logger
# for python3, to save on a text file all that is printed on the terminal:
import sys
...
# for python3, to save on a text file all that is printed on the terminal:
with open('Descriptors_terminal_text.txt', 'w') as sys.stdout:
...
parser.add_argument('-imgdir', nargs=1, type=str) #'./datasets/Mix/'
...
# Select model to use
load_model_dir  = args.modeldir[0] # './saved_models/vgg19/003_cub/'  # Model directory to ProtoPNet. 
load_model_name = args.model[0] # '999nopush0.7408.pth'# Model name of trained ProtoPNet

# Paths to dataset
dataset_dir = args.imgdir[0] # './datasets/Mix/' # Path to dataset
dataset = 'train' #'train_crop'
test_dataset = 'test' #'test_crop'

model_base_architecture = load_model_dir.split('/')[2:]
save_analysis_path = os.path.join('descriptors', model_base_architecture[0]) # os.path.join('pps_explanations', model_base_architecture, experiment_run, load_model_name)
...
prototypes = range(total_num_pps) #range(2000)

#max_count = 2
count=0
# Loop through image files in all image folders
for path, subdirs, files in os.walk(os.path.join(dataset_dir, dataset)):
    for subdir in subdirs:
        count+=1
        # if count > max_count:
        #     break
        # for python3, to save on a text file all that is printed on the terminal:
        with open('Descriptors_terminal_text', 'w') as sys.stdout:
            print("class: ", subdir, count, "/ ", num_classes, flush=True)
        for class_path, class_subdirs, class_files in os.walk(os.path.join(os.path.join(dataset_dir, dataset),subdir)):
...
with open(save_analysis_path + csv_file_name + 'trainingset_weighted_global_descriptors_score_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)), "w") as global_f:
...
with open(save_analysis_path + csv_file_name + 'trainingset_local_descriptors_score_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h), str(bright_lvl)), "w") as local_f:
...
count=0
# Loop through image files in all image folders
for path, subdirs, files in os.walk(os.path.join(dataset_dir, test_dataset)):
    for subdir in subdirs:
        count+=1
        # for python3, to save on a text file all that is printed on the terminal:
        with open('Descriptors_terminal_text', 'w') as sys.stdout:
            print("class: ", subdir, count, "/ ", num_classes, flush=True)
        for class_path, class_subdirs, class_files in os.walk(os.path.join(os.path.join(dataset_dir, test_dataset),subdir)):
            # Loop through files in folder
            for filename in class_files:
                img_path = os.path.join(class_path, filename) # Get path of file
                mod_tensors = []
...
# Convert results dataframe to csv format and display
with open(save_analysis_path + csv_file_name + 'testset_weighted_global_descriptors_score_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)), "w") as global_f:
    df_grouped_weighted_testset.reset_index()[['prototype', 'modification', 'weighted_delta']].to_csv(global_f, index=False)
# for python3, to save on a text file all that is printed on the terminal:
with open('Descriptors_terminal_text', 'w') as sys.stdout:
    print("Done with global scores. Now saving the local scores...", flush=True)
with open(save_analysis_path + csv_file_name + 'testset_local_descriptors_score_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h), str(bright_lvl)), "w") as local_f:
    results_testset.to_csv(local_f, index=False)
# for python3, to save on a text file all that is printed on the terminal:
with open('Descriptors_terminal_text', 'w') as sys.stdout:
    print("DONE!", flush=True)

#log, logclose = create_logger(log_filename=os.path.join(save_analysis_path, csv_file_name, 'Statistical_tests.log'))
#log('Dataframe head: \t{0}'.format(df_grouped_weighted.head()))
# for python3, to save on a text file all that is printed on the terminal:
with open('Descriptors_terminal_text', 'w') as sys.stdout:
...
scores_train = pd.read_csv(save_analysis_path + csv_file_name + 'trainingset_weighted_global_descriptors_score_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)))
scores_test = pd.read_csv(save_analysis_path + csv_file_name + 'testset_weighted_global_descriptors_score_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)))

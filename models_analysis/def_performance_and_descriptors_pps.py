...
def forward_particular_patch(ppnet, x, nearest_patch_indices):
    distances, conv_features = ppnet.prototype_distances(x) #shape (5,2000,7,7)
    ...
    return logits, patch_distances, prototype_similarities, conv_features
...
def get_descriptors(gpuid, modeldir, model, imgdir, csv_file_name):

    os.environ['CUDA_VISIBLE_DEVICES'] = gpuid # args.gpuid[0]

    # Select model to use 
    load_model_dir  = modeldir # args.modeldir[0] # './saved_models/vgg19/003_cub/'  # Model directory to ProtoPNet. 
    load_model_name = model # args.model[0] # '999nopush0.7408.pth'# Model name of trained ProtoPNet

    # Paths to dataset
    dataset_dir = imgdir # args.imgdir[0] # './datasets/Mix/' # Path to dataset
    dataset = 'train' #'train_crop'
    test_dataset = 'test' #'test_crop'

    model_base_architecture = load_model_dir.split('/')[5:]
    save_analysis_path = os.path.join('pps_results_and_descriptors_', model_base_architecture[0]) # os.path.join('pps_explanations', model_base_architecture, experiment_run, load_model_name)
        ...
        for subdir in subdirs:
            ...
            for class_path, class_subdirs, class_files in os.walk(os.path.join(os.path.join(dataset_dir, dataset),subdir)):
                ...
                for filename in class_files:
                    ...
                    with torch.no_grad():
                        logits, patch_distances, mod_similarities, conv_features = forward_particular_patch(ppnet, images_mod, nearest_patch_indices)
    ...
    with open(save_analysis_path +"/"+ csv_file_name + '_train_global_descriptors_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)), "w") as global_f:
    ...
    with open(save_analysis_path +"/"+ csv_file_name + '_train_locals_descriptors_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h), str(bright_lvl)), "w") as local_f:
            ...
            for class_path, class_subdirs, class_files in os.walk(os.path.join(os.path.join(dataset_dir, test_dataset),subdir)):
                ...
                for filename in class_files:
                    ...
                    for row in range(mod_similarities.shape[0]):
                        ...
                        results_testset = pd.concat([results_testset, df], axis=0, ignore_index = True)
    ...
    # Convert results dataframe to csv format and display
    with open(save_analysis_path +"/"+ csv_file_name + '_test_global_descriptors_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)), "w") as global_f:
        df_grouped_weighted_testset.reset_index()[['prototype', 'modification', 'weighted_delta']].to_csv(global_f, index=False)
    print("Done with global scores. Now saving the local scores...", flush=True)
    with open(save_analysis_path +"/"+ csv_file_name + '_test_locals_descriptors_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h), str(bright_lvl)), "w") as local_f:
    ...
    scores_train = pd.read_csv(save_analysis_path +"/"+ csv_file_name + '_train_global_descriptors_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)))
    scores_test = pd.read_csv(save_analysis_path +"/"+ csv_file_name + '_test_global_descriptors_%s_%s_%s_%s_%s_%s.csv'%(str(contr_lvl), str(sat_lvl), str(hue_lvl), str(mod_value_dict["shape"]), str(texture_h),str(bright_lvl)))
